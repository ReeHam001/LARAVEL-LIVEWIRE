<div>
    ...
    <div class="flex items-center justify-end py-4 text-right">
        <x-jet-button wire:click="showCreateModal"> {{-- from jetstream --}}
            {{ __('Create post') }}
        </x-jet-button>
    </div>
    ...
    <div class="pt-4">
        {{ $posts->links() }} {{-- for pagination in tailwind --}}
    </div>

    <x-jet-dialog-modal wire:model="modalFormVisible"> {{-- form --}}

        <x-slot name="title">
            {{ $modalId ? __('Update post') : __('Create post') }} {{-- modalId اذا كان موجود فهو للتعديل --}}
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <div class="mt-4">
                ...
                <x-jet-input typw="text" id="title" wire:model.debounce.500ms="title" class="block mt-1 w-full"></x-jet-input> {{-- debounce مشان يعطيني زمن تاخير بين ما يحدث العنوان لانه بدو ينبنى عليه --}}
                @error('title')<span class="text-red-900 text-sm font-extrabold">{{ $message }}</span>@enderror {{-- 500ms بينتظر نص ثانية --}}
            </div>
            ...
            <div class="mt-4">
                ...
                <div wire:ignore wire:key="myId"> {{--لانه جافا سكربت وبدنا نستخدم الايف واير تعريف الديف عن طريق الواير  --}}
                ...
                <textarea id="body" class="hidden body-content" wire:model.debounce.2000ms="body">
                    {!! $body !!}  {{-- wire:model.debounce ساوي تاخير مشان تجي بعد الكتابة --}}
                </textarea>
            ...
            <div class="mt-4">
                ...
                <div class="flex py-3">
                    @if ($image) {{-- ب edit اذا في صورة بيعرضها --}}
                    ...
                    @if ($post_image) {{-- الصورة بتظهر مباشرة قبل اجراء create --}}
                        <div class="mt-1 flex rounded-md shadow-sm">
                            <span class="inline-flex items-center p-3 rounded border border-gray-300 bg-gray-50 text-gray-500 text-sm">
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$toggle('modalFormVisible')">{{ __('Cancel') }}</x-jet-secondary-button> {{-- toogle العكس بالجافاسكربت --}}
...
@push('scripts')
    ...
    <script> /* لتشغيل ck editor */
        window.onload = function () { /* استخدام مكتبة جافاسكربت خارجية */
            if (document.querySelector('#body')) {
                ...
                .then(editor => {
                    ...
                    Livewire.on('createNewPostEmit', function () {
                        editor.setData('')  // مشان يفضي التيكست ايريا
                    });
